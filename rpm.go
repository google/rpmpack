...
func (r *RPM) SetScriptletInterpreterFor(name, interpreter string) error {
	...
	content := ""
	item, found := r.scriptlets[name]
	if found {
		// keep previously set script content
		content = item.content
	}
	r.scriptlets[name] = Scriptlet{content, interpreter}
	return nil
}

func (r *RPM) setScriptlet(name, s string) {
	_, found := scriptletsTypes[name]
	if !found {
		panic(fmt.Sprintf("internal error: invalid scriptlet name %q", name))
	}
	...
	item, found := r.scriptlets[name]
	if found {
		// keep previously set interpreter
